...
export async function getResource(ctx: QueryCtx, id: Id<"resources">) {
...
export async function updateResource(
	...
	for (const [key, value] of Object.entries(updates)) {
		if (
			JSON.stringify(resource[key as keyof typeof resource]) !==
...
export async function searchResources(
	...
	const query = ctx.db.query("resources").withSearchIndex("search", (q) => {
...
export async function getFavoriteResources(ctx: QueryCtx, userId: Id<"users">) {
...
export async function leaveCollaborationSession(
	...
	const participants = session.participants.map((p) =>
		p.userId === userId && !p.leftAt ? { ...p, leftAt: Date.now() } : p,
	);
