...
export const getResourcesByType = query({
	...
	returns: v.array(resourceValidator),
	handler: async (ctx, args) => {
		return await ResourceModel.getResourcesByType(
			ctx,
			args.projectId,
			args.type,
		);
...
export const createResource = zodMutation({
	args: createResourceSchema,
	returns: zid("resources"),
	handler: async (ctx, args) => {
		const user = (await ctx.runQuery(api.users.getCurrentUser)) as Doc<"users">;
...
export const shareResource = mutation({
	...
	returns: v.id("resources"),
	handler: async (ctx, args) => {
		return await ResourceModel.shareResource(
			ctx,
			args.resourceId,
			args.visibility,
		);
...
export const getResourceAccessHistory = query({
	...
	handler: async (ctx, args) => {
		return await ResourceModel.getResourceAccessHistory(
			ctx,
			args.resourceId,
			args.limit,
		);
...
export const getSharedWithMe = query({
	...
	handler: async (ctx, args) => {
		return await ResourceModel.getSharedWithMePaginated(
			ctx,
			args.paginationOpts,
		);
	},
...
export const getActiveCollaborationSessions = query({
	...
	handler: async (ctx, args) => {
		return await ResourceModel.getActiveCollaborationSessions(
			ctx,
			args.resourceId,
		);
	},
