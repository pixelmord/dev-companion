  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <h4 className={`text-white font-medium truncate max-w-[75%] ${compact ? 'text-sm' : 'text-base'}`}>
          {task.title}
        </h4>
        <div className={`bg-retro-dark px-1.5 py-0.5 rounded text-white/70 text-xs ${compact ? 'hidden' : ''}`}>
          {task.project}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-1">
        <div className={`flex items-center text-white/50 ${compact ? 'text-xs' : 'text-xs'}`}>
