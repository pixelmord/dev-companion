...
const PokerCard: React.FC<PokerCardProps> = ({
	...
	let cardColors = {
		...
		bgGradient: isDarkMode
			? "from-retro-dark to-retro-medium"
	...
	return (
      ...
      `}
		>
			{/* Card corners */}
			<div
				className={`absolute top-2 left-2 ${cardColors.textColor} font-bold text-lg`}
			>
				{value}
			</div>
			<div
				className={`absolute bottom-2 right-2 ${cardColors.textColor} font-bold text-lg rotate-180`}
			>
				{value}
			</div>
			...
			<div className="absolute inset-0 flex items-center justify-center">
				...
				{type === "mystery" && (
					<div className={`text-5xl font-bold ${cardColors.textColor}`}>?</div>
				)}
			</div>

			{/* Card decoration */}
			<div className="absolute inset-3 border border-dashed border-opacity-30 rounded pointer-events-none" />

			{/* Selection indicator */}
			{isSelected && (
				<div className="absolute -top-1 -right-1 h-4 w-4 bg-neon-cyan rounded-full animate-pulse" />
			)}
